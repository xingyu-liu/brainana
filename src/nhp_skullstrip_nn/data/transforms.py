...
class RandomRotation:
    ...
    def _rotate_tensor(
        self, tensor: torch.Tensor, angle: float, mode: str
    ) -> torch.Tensor:
        ...
        needs_dtype_conversion = original_dtype in [
            torch.long,
            torch.int,
            torch.int64,
            torch.int32,
        ...
        rotation_matrix = torch.tensor(
            [[cos_val, -sin_val, 0], [sin_val, cos_val, 0]], dtype=torch.float32
        )
        ...
        grid = F.affine_grid(
            rotation_matrix.unsqueeze(0), tensor.size(), align_corners=False
        )

        rotated = F.grid_sample(
            tensor, grid, mode=mode, align_corners=False, padding_mode="border"
        )
...
class RandomScale:
    ...
    def _scale_tensor(
        self, tensor: torch.Tensor, scale: float, mode: str
    ) -> torch.Tensor:
        ...
        scale_matrix = torch.tensor([[scale, 0, 0], [0, scale, 0]], dtype=torch.float32)
        ...
        grid = F.affine_grid(
            scale_matrix.unsqueeze(0), tensor.size(), align_corners=False
        )
...
class RandomTranslation:
    ...
    def _translate_tensor(
        self, tensor: torch.Tensor, tx: float, ty: float, mode: str
    ) -> torch.Tensor:
        ...
        translation_matrix = torch.tensor(
            [[1, 0, norm_tx], [0, 1, norm_ty]], dtype=torch.float32
        )
        ...
        grid = F.affine_grid(
            translation_matrix.unsqueeze(0), tensor.size(), align_corners=False
        )
...
class RandomBrightnessContrast:
    ...
    def __call__(
        ...
        if isinstance(data, list):
            ...
            result = data.copy()
            result[0] = self._adjust_brightness_contrast(
                result[0], brightness, contrast
            )
            return result
        else:
    ...
    def _adjust_brightness_contrast(
        self, tensor: torch.Tensor, brightness: float, contrast: float
    ) -> torch.Tensor:
...
class RandomFieldBias:
    ...
    def _generate_random_bias_field(self, shape: Tuple[int, ...]) -> torch.Tensor:
        ...
        if len(shape) == 2:
            # 2D case
            bias_field = self._generate_2d_bias_field(
                shape, bias_strength, spatial_freq, num_peaks
            )
        elif len(shape) == 3:
            # 3D case - generate 2D bias field and apply to each slice
            bias_field_2d = self._generate_2d_bias_field(
                shape[1:], bias_strength, spatial_freq, num_peaks
            )
            bias_field = bias_field_2d.unsqueeze(0).expand(shape[0], -1, -1)
        else:
            # Higher dimensional case - treat last two dimensions as spatial
            bias_field_2d = self._generate_2d_bias_field(
                shape[-2:], bias_strength, spatial_freq, num_peaks
            )
            bias_field = bias_field_2d.unsqueeze(0).expand(*shape[:-2], -1, -1)

        return bias_field

    def _generate_2d_bias_field(
        self,
        shape: Tuple[int, int],
        bias_strength: float,
        spatial_freq: float,
        ...
        spatial_variation = 0.3 * torch.sin(
            2 * np.pi * freq_x * X + phase_x
        ) + 0.3 * torch.cos(2 * np.pi * freq_y * Y + phase_y)

        bias_field += spatial_variation

        # Normalize to [1 - bias_strength, 1 + bias_strength]
        bias_field = (bias_field - bias_field.min()) / (
            bias_field.max() - bias_field.min()
        )
...
def create_training_transforms(
    ...
    if enable_translation:
        transforms.append(
            RandomTranslation(
                translation_range=translation_range, prob=prob_translation
            )
        )
...
def create_transforms_from_config(augmentation_config: dict) -> Compose:
    ...
    return create_training_transforms(
        ...
        translation_range=to_tuple(
            augmentation_config.get("translation_range"), (15.0, 15.0)
        ),
        ...
        bias_strength_range=to_tuple(
            augmentation_config.get("bias_strength_range"), (0.1, 0.4)
        ),
        spatial_freq_range=to_tuple(
            augmentation_config.get("spatial_freq_range"), (0.3, 0.8)
        ),
