...
def write_nifti(
    ...
    if shape is not None:
        if len(shape) != len(data.shape):
            raise ValueError(
                f"Shape mismatch: data has {len(data.shape)} dimensions, "
                f"but shape specifies {len(shape)} dimensions"
            )

        # Crop data to specified shape
        slices = tuple(
            slice(0, min(data.shape[i], shape[i])) for i in range(len(shape))
        )
...
def load_nifti(
    file_path: Union[str, Path], normalize: bool = True, dtype: np.dtype = np.float32
...
def validate_nifti_compatibility(
    file1_path: Union[str, Path], file2_path: Union[str, Path]
) -> bool:
    """
