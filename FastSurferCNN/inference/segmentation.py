...
"""
Segmentation interface for FastSurferCNN.

This module provides a simple interface for multi-class atlas segmentation using FastSurferCNN,
which generates segmentation, brain mask, and hemisphere mask outputs.
"""
...
from FastSurferCNN.inference.api import segmentation
...
def run_segmentation(
    ...
    Args:
        ...
        use_mixed_model: bool, default=False
            ...
            {modal}_seg-{atlas}_mixed.pkl (e.g., T1w_seg-ARM2_mixed.pkl).
    ...
    logger.info(f"Segmentation (FastSurferCNN): starting for {modal} modality")
    
    # Validate inputs
    input_image = Path(input_image)
    if not input_image.exists():
        logger.error(f"Segmentation (FastSurferCNN): input image not found: {input_image}")
    ...
    if modal != 'anat':
        logger.error(f"Segmentation (FastSurferCNN): invalid modality={modal}, must be 'anat'")
    ...
    try:
        ...
        logger.info("Segmentation (FastSurferCNN): completed successfully")
        ...
        logger.error(f"Segmentation (FastSurferCNN): failed: {str(e)}")
        raise RuntimeError(f"Segmentation (FastSurferCNN) failed: {str(e)}") from e
