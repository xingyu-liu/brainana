...
def get_default_config() -> Dict[str, Any]:
    ...
    if defaults_path_yaml.exists():
        config = load_yaml_config(defaults_path_yaml)
    elif defaults_path_json.exists():
        config = load_json_config(defaults_path_json)
    else:
        raise FileNotFoundError(
            f"Default configuration file not found. Expected defaults.yaml or defaults.json in {Path(__file__).parent}"
        )
...
def load_config(config_path: Optional[Union[str, Path]] = None) -> Dict[str, Any]:
    ...
    if config_path is not None:
        ...
        if suffix in [".yaml", ".yml"]:
            ...
            raise ValueError(
                f"Unsupported config file format: {suffix}. Supported: .yaml, .yml, .json, .py"
            )
...
def save_config(config: Dict[str, Any], output_path: Union[str, Path]) -> None:
    ...
    try:
        if suffix in [".yaml", ".yml"]:
            ...
            if suffix:
                raise ValueError(
                    f"Unsupported output format: {suffix}. Use .yaml, .yml, or .json"
                )
