...
class TrainingPlotter:
    ...
    def _plot_learning_rate(self, df: pd.DataFrame, ax=None, save_individual=True):
        ...
        if "lr" in df.columns:
            ...
            ax.plot(
                df["epoch"], df["lr"], self.colors["single"], linewidth=2, alpha=0.8
            )
            ax.set_xlabel("Epoch")
...
class PlottingCallback:
    ...
    def on_epoch_end(self, epoch: int, logs: Dict[str, float], trainer):
        ...
        if hasattr(trainer, "device") and trainer.device.type == "cuda":
            try:
                gpu_memory = torch.cuda.memory_allocated(trainer.device) / (
                    1024**3
                )  # Convert to GB
...
def create_training_summary(config, training_history, output_dir):
    ...
    with open(summary_file, "w") as f:
        ...
        if training_history:
            ...
            f.write(
                f"Best validation loss: {best_val_loss:.6f} (epoch {best_epoch + 1})\n"
            )
            f.write(
                f"Best validation Dice: {best_val_dice:.6f} (epoch {best_epoch + 1})\n\n"
            )
