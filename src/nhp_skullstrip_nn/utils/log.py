...
class MacacaLogger:
    ...
    def __init__(
        self, name: str, output_dir: Optional[str] = None, log_level: str = "INFO"
    ):
...
def log_model_info(
    logger: logging.Logger, model: object, device: str, dataset_size: int
):
...
def log_gpu_info(logger: logging.Logger):
    ...
    try:
        ...
        for i in range(torch.cuda.device_count()):
            ...
            logger.info(
                f"  GPU {i}: {gpu_name} - {memory_allocated:.1f}GB / {memory_total:.1f}GB allocated"
            )
...
def log_data_info(
    logger: logging.Logger, image_files: list, label_files: list, missing_count: int = 0
):
