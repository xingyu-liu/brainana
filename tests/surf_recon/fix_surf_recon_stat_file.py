...
replace_dict = {"SII": "SI/SII", "STSd": "STG/STSd", "belt": "core/belt"}
            ...
            for key, value in replace_dict.items():
                if key in stats_file.read_text():
                    if value not in stats_file.read_text():
                        ...
                        print(
                            f"{site_name} / {sub} / {hemi} stats file already has {value}"
                        )
                else:
                    print(
                        f"{site_name} / {sub} / {hemi} stats file does not have {key}"
                    )
