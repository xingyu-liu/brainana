"""
Environment and dependency checking for nhp_mri_prep.
...
def check_package_version(
    ...
    try:
        ...
        is_compatible = version.parse(installed_version_clean) >= version.parse(
            min_version
        )
...
def extract_version_number(version_string: str) -> Optional[str]:
    ...
    if all_matches:
        ...
        all_matches_sorted = sorted(
            all_matches, key=lambda x: (len(x), x), reverse=True
        )
...
def check_external_tool(tool_name: str, tool_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if version_cmd:
        ...
        if is_available:
            ...
            if not any(
                c["command"] == version_cmd_name for c in result["commands_found"]
            ):
                result["commands_found"].append(
                    {"command": version_cmd_name, "path": cmd_path}
                )
            ...
            if version_info:
                ...
                if installed_version:
                    ...
                    if required_version:
                        ...
                        try:
                            result["version_matches"] = version.parse(
                                installed_version
                            ...
                            result["errors"].append(
                                f"Error comparing versions: {str(e)}"
                            )
                    else:
                    ...
                    result["errors"].append(
                        f"Could not extract version number from: {version_info}"
                    )
            else:
                if not version_cmd_name or not any(
                    c["command"] == version_cmd_name for c in result["commands_found"]
                ):
                    result["errors"].append(
                        f"Version command '{version_cmd_name}' not found in PATH"
                    )
                else:
                    result["errors"].append(
                        f"Could not get version info using: {version_cmd}"
                    )
    ...
    if len(commands) > 0:
        ...
        commands_ok = (
            any(c["command"] == version_cmd_name for c in result["commands_found"])
            and result["version_info"] is not None
        )
    else:
    ...
    if not result["available"] and not result["env_var_set"] and env_var:
        ...
        if len(result["commands_found"]) == 0 and (
            not version_cmd or not result["version_info"]
        ):
            result["errors"].append(
                f"Environment variable {env_var} not set and tool not found in PATH"
            )
    ...
    if not result["env_var_set"] and result["available"] and env_var:
        logger.warning(
            f"System: {tool_name} available via PATH but environment variable {env_var} not set"
        )

    logger.debug(
        f"System: tool check for {tool_name} - {'✓' if result['available'] else '✗'}"
    )
...
def check_dependencies(
    include_optional: bool = True, logger: Optional[logging.Logger] = None
) -> Dict[str, Any]:
    ...
    result["python"] = {"version": py_version, "compatible": is_compatible}

    if not is_compatible:
        min_py_version = "3.11"  # Match pyproject.toml requires-python
        result["errors"].append(
            f"Python version {py_version} < required {min_py_version}"
        )
        logger.error(
            f"System: incompatible Python version - {py_version} < {min_py_version}"
        )

    # Check required packages
    for pkg_name, min_version in REQUIRED_PYTHON_PACKAGES.items():
        is_available, installed_version, error_msg = check_package_version(
        ...
        if not is_available:
            ...
            if error_msg:
                result["errors"].append(
                    f"Required package issue: {pkg_name} - {error_msg}"
                )
                logger.error(
                    f"System: required package missing - {pkg_name} (>= {min_version})"
                )
                print(f"Required package missing: {pkg_name} (>= {min_version})")
        else:
            logger.debug(
                f"System: required package OK - {pkg_name} {installed_version}"
            )
    ...
    if include_optional:
        for pkg_name, min_version in OPTIONAL_PYTHON_PACKAGES.items():
            ...
            if not is_available:
                ...
                if error_msg:
                    logger.debug(
                        f"System: optional package missing - {pkg_name} - {error_msg}"
                    )
            else:
                logger.debug(
                    f"System: optional package OK - {pkg_name} {installed_version}"
                )
...
def check_environment(
    logger: Optional[logging.Logger] = None, config: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    ...
    for tool_name, tool_config in REQUIRED_EXTERNAL_TOOLS.items():
        ...
        if not tool_result["available"]:
            ...
            result["summary"]["critical_issues"].append(
                f"External tool not available: {tool_name}"
            )
    ...
    if config is not None:
        ...
        anat_method = (
            anat_ss.get("method", "").lower() if anat_ss.get("enabled", False) else None
        )
        func_method = (
            func_ss.get("method", "").lower() if func_ss.get("enabled", False) else None
        )
        ...
        if (anat_fscnn or func_fscnn) and not cuda_available:
            ...
            warn_msg = (
                f"GPU (CUDA) not available, but skullstripping with fastsurfer_nn is enabled for "
    ...
    if critical_count == 0 and warning_count == 0:
        ...
        logger.error(
            f"System: ✗ environment check failed - {critical_count} critical issues found"
        )
        print("✗ Environment check failed - critical issues found")
...
def print_environment_report(env_results: Dict[str, Any]) -> None:
    ...
    for tool_name, tool_info in tools.items():
        ...
        if tool_info["installed_version"]:
            ...
            print(
                f"      ver: {tool_info['installed_version']} (required: {required}) {version_status}"
            )
    ...
    if "total_gb" in resources["memory"]:
        ...
        print(
            f"  Memory: {mem['available_gb']:.1f}GB available / {mem['total_gb']:.1f}GB total"
        )
...
def info() -> str:
    ...
    status = (
        "OK" if python_compatible and packages_ok == len(key_packages) else "ISSUES"
    )
