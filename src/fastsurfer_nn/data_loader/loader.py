...
from fastsurfer_nn.data_loader import dataset as dset
from fastsurfer_nn.data_loader.data_transforms import AddGaussianNoise, ToTensor, Pad2D
    ...
    if mode == "train":
        if "None" in cfg.DATA.AUG:
            ...
            if hasattr(cfg.DATA, "AUG_ELASTIC"):
                elastic_num_points = getattr(
                    cfg.DATA.AUG_ELASTIC, "NUM_CONTROL_POINTS", 5
                )
                elastic_max_disp = getattr(
                    cfg.DATA.AUG_ELASTIC, "MAX_DISPLACEMENT", (20, 20, 0)
                )
            else:
            ...
            if hasattr(cfg.DATA, "AUG_TRANSLATION"):
                translation_values = getattr(
                    cfg.DATA.AUG_TRANSLATION, "TRANSLATION", (15.0, 15.0, 0)
                )
            else:
            ...
            if hasattr(cfg.DATA, "AUG_RANISOTROPY"):
                ranisotropy_downsampling = getattr(
                    cfg.DATA.AUG_RANISOTROPY, "DOWNSAMPLING", (1.1, 1.3)
                )
                ranisotropy_axes = getattr(cfg.DATA.AUG_RANISOTROPY, "AXES", (0, 1))
            else:
                ranisotropy_downsampling = (
                    1.1,
                    1.3,
                )  # Reduced from (1.1, 1.5) - less aggressive = faster
            ...
            if hasattr(cfg.DATA, "AUG_BIASFIELD"):
                biasfield_coefficients = getattr(
                    cfg.DATA.AUG_BIASFIELD, "COEFFICIENTS", (0.3, 0.7)
                )
            ...
            bias_field = tio.transforms.RandomBiasField(
                coefficients=biasfield_coefficients,
                order=biasfield_order,
                include=["img"],
            )

            # Gamma - Can be configured via cfg.DATA.AUG_RGAMMA if needed
            if hasattr(cfg.DATA, "AUG_RGAMMA"):
                rgamma_log_gamma = getattr(
                    cfg.DATA.AUG_RGAMMA, "LOG_GAMMA", (-0.1, 0.1)
                )
            else:
            ...
            if hasattr(cfg.DATA, "AUG_PROBABILITIES"):
                try:
                    ...
                    if hasattr(aug_probs_dict, "__dict__"):
                        aug_probs = {
                            k: getattr(aug_probs_dict, k)
                            for k in dir(aug_probs_dict)
            ...
            cheap_geometric = [
                ...
                "Scaling",
                "Translation",
            ]  # Fast: simple affine transforms
            expensive_geometric = [
                "Elastic",
                "RAnisotropy",
            ]  # Slow: complex deformations
            ...
            for aug in cfg.DATA.AUG:
                ...
                if aug not in all_augs:
                    logger.warning(
                        f"Augmentation '{aug}' not found in available augmentations. Skipping."
                    )
                    continue
            ...
            if transform_list:
                transform = tio.Compose(
                    transform_list, include=["img", "label", "weight"]
                )
            else:
