...
class TrainingConfig:
    ...
    dataset: str                           # Dataset: directory path or JSON file path
    ...
    def __post_init__(self):
        ...
        if not os.path.exists(self.dataset):
            raise FileNotFoundError(f"Dataset path not found: {self.dataset}")
        
        # Construct full output directory path
        if self.experiment_name:
            self.output_dir = f"{self.output_base_dir}/{self.modal}_{self.label}_{self.experiment_name}"
        else:
            self.output_dir = f"{self.output_base_dir}/{self.modal}_{self.label}"
        
        # Note: Simplified logging - no longer using tensorboard or advanced logging features
    ...
    def get_data_paths(self) -> Tuple[List[str], List[str]]:
        ...
        if self.dataset.endswith('.json'):
            ...
            if 'images' not in data or 'labels' not in data:
                raise ValueError(f"JSON file must contain 'images' and 'labels' keys: {self.dataset}")
            ...
            # Directory format - assume dataset contains 'images' and 'labels' subdirectories
            images_dir = os.path.join(self.dataset, 'images')
            labels_dir = os.path.join(self.dataset, 'labels')
            ...
            if not valid_pairs:
                raise ValueError(f"No valid image-label pairs found in {self.dataset}")
