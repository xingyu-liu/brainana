"""
Template management utilities for nhp_mri_prep.
...
class TemplateManager:
    ...
    def __init__(self, template_dir: Optional[str] = None):
        ...
        self.defaults = {"template": "NMT2Sym", "res": "res-05", "desc": "brain"}
    ...
    def _discover_templates(self) -> Dict[str, Dict[str, str]]:
        ...
        for template_file in template_root.glob("**/*.nii.gz"):
            ...
            if parsed:
                ...
                logger.debug(
                    f"Data: found template - {template_name} -> {spec} -> {template_file.name}"
                )

        return dict(templates)

    def _parse_template_filename(
    ...
    def list_available_templates(self) -> List[str]:
        ...
        for template_name, specs in self._templates.items():
            ...
            filtered_specs = [
                spec for spec in specs.keys() if not spec.startswith("_components_")
            ]
    ...
    def resolve_template(self, template_spec: Optional[str] = None) -> str:
        ...
        if template_name not in self._templates:
            ...
            raise ValueError(
                f"Template '{template_name}' not found. Available templates: {available_names}"
            )

        # Get all specifications for this template
        available_specs = {
            k: v
            for k, v in self._templates[template_name].items()
            if not k.startswith("_components_")
        }

        # Find best match using component-based scoring algorithm
        best_match = self._find_best_match_by_components(
            template_name, requested_components, available_specs
        )

        if best_match:
            logger.debug(
                f"System: resolved template spec '{template_spec}' to best match '{best_match}'"
            )
        ...
        raise ValueError(
            f"No suitable match found for '{template_spec}'. Available options for {template_name}: {available_specs_list}"
        )

    def _find_best_match_by_components(
        self,
        template_name: str,
        requested_components: Dict[str, str],
        available_specs: Dict[str, str],
    ) -> Optional[str]:
        ...
        for spec, _ in available_specs.items():
            ...
            if self._meets_all_requirements(requested_components, spec_components):
                score = self._calculate_component_score(
                    requested_components, spec_components
                )
                valid_candidates.append((score, spec))
                logger.debug(
                    f"Data: valid candidate {spec} - components={spec_components}, score={score}"
                )
            else:
                logger.debug(
                    f"Data: rejected candidate {spec} - missing required components"
                )
    ...
    def _meets_all_requirements(
        self, requested: Dict[str, str], available: Dict[str, str]
    ) -> bool:
    ...
    def _calculate_component_score(
        ...
        if (
            "res" not in requested
            and "res" in available
            and available["res"] == "res-05"
        ):
            score += 20

        # If user didn't specify a description, prefer brain
        if (
            "desc" not in requested
            and "desc" in available
            and available["desc"] == "brain"
        ):
            score += 20
