...
class UNet2d(nn.Module):
        ...
        for n in range(num_conv_block):
            if n == 0:
                setattr(
                    self,
                    "conv%d" % (n + 1),
                    Conv2dBlock(dim_in, kernel_root, use_inst_norm=use_inst_norm),
                )
            else:
                setattr(
                    self,
    ...
    def forward(self, x):
        ...
        decoder_out = conv_out["conv%d" % num_conv_block]
        for n in range(num_conv_block - 1):
            i = num_conv_block - 1 - n
            up = getattr(self, "upconv%dto%d" % (i + 1, i))(
                conv_out["conv%d" % (i + 1)] if n == 0 else decoder_out
            )
            tmp = torch.cat((up, conv_out["conv%d" % i]), 1)
            decoder_out = getattr(self, "conv%dm" % (i))(tmp)
