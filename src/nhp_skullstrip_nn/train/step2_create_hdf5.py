...
"""
Generate HDF5 training dataset for nhp_skullstrip_nn.
...
def resize_volume_proportional(volume, target_size=256, order=1):
    ...
    resized = F.interpolate(
        volume_tensor, size=new_shape, mode=mode, align_corners=align_corners
    )
...
def filter_blank_slices(image_slices, label_slices, threshold=10):
    ...
    if np.sum(valid_mask) == 0:
        return np.zeros(
            (image_slices.shape[0], image_slices.shape[1], 0, image_slices.shape[3])
...
def extract_slices_from_volume(volume, label, num_slices=3, rescale_dim=256):
    ...
    volume_resized, scale_factor = resize_volume_proportional(
        volume, rescale_dim, order=1
    )
    ...
    for slice_range in slice_list_0:
        ...
        # Pad to rescale_dim
        image_block = torch.zeros(
            [num_slices, rescale_dim, rescale_dim], dtype=torch.float32
        )
        label_block = torch.zeros(
            [num_slices, rescale_dim, rescale_dim], dtype=torch.long
        )
        ...
        if torch.sum(label_block > 0) > 10:
            ...
            all_label_slices.append(
                label_block[0].numpy()
            )  # Use middle slice for label
...
def process_subject(
    image_path, label_path, num_slices=3, rescale_dim=256, verbose=False
):
    ...
    try:
        ...
        assert (
            images_array.shape[2] == rescale_dim
            and images_array.shape[3] == rescale_dim
        )
        assert (
            labels_array.shape[1] == rescale_dim
            and labels_array.shape[2] == rescale_dim
        )
        ...
        try:
            with h5py.File(output_hdf5, "r") as hf_check:
                if "images" in hf_check and "subject" in hf_check:
                    ...
                    print(
                        "  ⚠️  Existing file doesn't contain expected datasets, will overwrite"
                    )
...
def create_hdf5_dataset(
    ...
    print(
        f"Memory: {mem.available / (1024**3):.1f} GB available ({100-mem.percent:.1f}% free)"
    )
    if mem.percent > 70:
    ...
    already_processed, file_mode = _check_resume_capability(
        output_hdf5, num_slices, rescale_dim
    )
    ...
    if subject_filter is not None:
        ...
        print(
            f"Filtered {len(image_files)}/{len(already_processed) + len(image_files)} subjects based on data split"
        )
    ...
    with h5py.File(output_hdf5, file_mode) as hf:
        ...
        if "images" not in hf:
            ...
            images_ds = _create_resizable_dataset(
                hf, "images", np.float32, initial_shape_img
            )
            labels_ds = _create_resizable_dataset(
                hf, "labels", np.int64, initial_shape_label
            )
            subjects_ds = hf.create_dataset(
                "subject",
                (0,),
                maxshape=(None,),
        ...
        try:
            if num_workers > 1:
                process_args = [
                    ...
                    for idx, (img_file, label_file) in enumerate(
                        zip(image_files, label_files), 1
                    )
                ]
                ...
                for idx, (img_file, label_file) in enumerate(
                    ...
                    subject_name = (
                        Path(img_file).stem.replace(".nii", "").replace(".gz", "")
                    )
                    ...
                    subjects_processed_set.add(subject_name)
                    print(
                        f"Processing ({idx}/{total_subjects}): {subject_name} → {result['num_slices']} slices"
                    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate HDF5 dataset for nhp_skullstrip_nn"
    )
    parser.add_argument(
    ...
    num_workers = (
        ...
        else getattr(config, "num_workers", 8)
    )
