...
def prepare_data_loaders(config, logger: Optional[logging.Logger] = None):
    ...
    if getattr(config, "test_split", 0) > 0:
        ...
        val_size_adjusted = getattr(config, "validation_split", 0.2) / (
            1 - config.test_split
        )
        ...
        logger.info(
            f"Data split: Train={len(train_imgs)}, Val={len(val_imgs)}, Test={len(test_imgs)}"
        )
    ...
    val_dataset = FileListDataset(
        list(val_imgs), list(val_labels), config.num_input_slices, config.rescale_dim
    )

    # Data loader settings
    batch_size = getattr(config, "batch_size", 4)
    num_workers = (
        getattr(config, "num_workers", 8) if getattr(config, "num_workers", 8) else 0
    )
    ...
    return (
        ...
        val_loader,
        test_loader,
        (test_imgs, test_labels) if test_imgs else None,
    )


def _prepare_hdf5_data_loaders(
    config, train_hdf5, val_hdf5, logger: Optional[logging.Logger] = None
):
...
def adjust_batch_size_during_training(
    ...
    gpu_memory_threshold = getattr(
        config, "gpu_memory_threshold", 0.85
    )  # 85% of GPU memory
    ...
    if "gpu_allocated" in memory_usage:
        ...
        if memory_usage_ratio > gpu_memory_threshold:
            ...
            if logger and new_batch_size != current_batch_size:
                logger.warning(
                    f"GPU memory usage high ({memory_usage_ratio:.1%}), "
        ...
        # Check if we can increase batch size (memory usage is low)
            ...
            if logger and new_batch_size != current_batch_size:
                logger.info(
                    f"GPU memory usage low ({memory_usage_ratio:.1%}), "
