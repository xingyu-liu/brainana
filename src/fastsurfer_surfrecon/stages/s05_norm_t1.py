    ...
    def _run(self) -> None:
        ...
        if self.config.processing.get_t1:
            if not t1.exists():
                ...
                aseg_orig = self.sd.mri(
                    f"aparc.{self.config.atlas.name}atlas+aseg.orig.mgz"
                )
                mri_normalize(
                    input_vol=nu,
                    output_vol=t1,
            ...
            if brainmask.exists() and brainmask.is_symlink():
                logger.info(
                    "Removing symlink brainmask.mgz (will recreate from T1.mgz)"
                )
                brainmask.unlink()
            ...
            if not t1.exists():
                logger.info(
                    "Linking T1.mgz to nu.mgz (skipping normalization, keeping skull)"
                )
            ...
            if brainmask.exists() and not brainmask.is_symlink():
                logger.info(
                    "Removing existing brainmask.mgz (will create symlink to norm.mgz)"
                )
                brainmask.unlink()
