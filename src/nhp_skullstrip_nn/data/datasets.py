...
def validate_labels(
    label_data: np.ndarray, num_classes: int, dataset_name: str = "dataset"
) -> np.ndarray:
    ...
    if min_label < 0:
        print(
            f"Warning: Found negative labels in {dataset_name}. Setting negative values to 0."
        )
    ...
    if max_label >= num_classes:
        print(
            f"Warning: Found labels up to {max_label} in {dataset_name}, but num_classes={num_classes}"
        )
...
class VolumeDataset(data.Dataset):
    ...
    def __init__(self, input_image=None, input_label=None, debug=True):
    ...
    def __getitem__(self, index):
        ...
        if self.image_path is not None:
            ...
            if image.ndim == 4:
                print("Warning: 4D image detected. Averaging the last dimension.")
        ...
        if self.label_path is not None:
            ...
            if label.ndim == 4:
                print(
                    "Warning: 4D label detected. Taking the first slice of the last dimension."
                )
...
class BlockDataset(data.Dataset):
    ...
    def __init__(self, image=None, label=None, num_slice=3, rescale_dim=256):
        ...
        if image.dim() == 3:
            ...
            raise ValueError(
                f"Expected 3D or 4D tensor, got {image.dim()}D tensor with shape {image.shape}"
            )
        ...
        uns_image = nn.functional.interpolate(
            uns_image,
        ...
        if isinstance(label, torch.Tensor):
            ...
            if label.dim() == 3:
                ...
                raise ValueError(
                    f"Expected 3D or 4D label tensor, got {label.dim()}D tensor with shape {label.shape}"
                )
            ...
            uns_label = nn.functional.interpolate(
                uns_label, scale_factor=rescale_factor, mode="nearest"
            )
        ...
        rescale_shape = image.shape[
            1:
        ]  # Skip batch dimension to get [height, width, depth]
        ...
        )  # Total slices across all orientations
    ...
    def get_one_directory(self, axis=0):
        ...
        if axis == 0:
            ...
            ind = range(
                len(self.slice_list_0), len(self.slice_list_0) + len(self.slice_list_1)
            )
            slice_list = self.slice_list_1
        elif axis == 2:
            # Coronal slices (third dimension)
            ind = range(
                len(self.slice_list_0) + len(self.slice_list_1),
                len(self.slice_list_0)
                + len(self.slice_list_1)
                + len(self.slice_list_2),
            )
    ...
    def __getitem__(self, index):
        ...
        # ===== CORONAL SLICES (third dimension) =====
            ...
            slice_range = self.slice_list_2[
                slice_idx - len(self.slice_list_0) - len(self.slice_list_1)
            ]
        ...
        slice_shape = image_tmp.shape[
            1:
        ]  # Skip the num_slice dimension to get [height, width]
        ...
        if isinstance(self.label, torch.Tensor) and label_tmp is not None:
            ...
            if self.transform is not None:
                ...
                try:
                    ...
                    warnings.warn(
                        f"Transform failed: {e}. Continuing without transforms."
                    )
