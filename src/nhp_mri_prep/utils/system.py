"""
System utilities for nhp_mri_prep.
...
def _get_numerical_thread_env_vars(
    threads: int, max_threads: int = 32, include_itk: bool = True
...
def set_numerical_threads(
    ...
    """
    env_vars, num_threads = _get_numerical_thread_env_vars(
        threads, max_threads, include_itk
    )
...
def check_dependency(
    command: str, step_logger: Optional[logging.Logger] = None
) -> bool:
    ...
    try:
        ...
        if available:
            cmd_logger.debug(
                f"System: dependency check passed - {command} is available"
            )
        else:
            cmd_logger.debug(
                f"System: dependency check failed - {command} not available"
            )
        return available
...
def run_command(
    ...
    try:
        ...
        cmd_logger.info(
            f"System: command completed - exit code {result.returncode}, duration {execution_time:.2f}s"
        )
