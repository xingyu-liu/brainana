...
class Trainer:
    ...
    def __init__(self, config: TrainingConfig):
        ...
        self.logger = setup_logging(
            config.output_dir, getattr(config, "log_level", "INFO")
        )
        ...
        self.val_metrics = MetricsTracker(
            num_classes=num_classes, device=self.device, use_argmax=True
        )
            ...
            callbacks.append(
                EarlyStopping(
                    ...
                    restore_best_weights=getattr(
                        self.config, "early_stopping_restore_best_weights", True
                    ),
                )
            )
        ...
        try:
            for epoch in range(self.config.num_epochs):
                ...
                if epoch < self.warmup_epochs:
                    ...
                    self.logger.debug(
                        f"Warmup epoch {epoch+1}/{self.warmup_epochs}: LR = {current_lr:.2e}"
                    )
                else:
                    ...
                    if epoch == self.warmup_epochs:
                        ...
                        if getattr(self.config, "use_cosine_scheduler", False):
                            effective_epochs = (
                                self.config.num_epochs - self.warmup_epochs
                            )
                            ...
                            self.logger.info(
                                f"Warmup complete: LR = {self.base_lr:.2e}, cosine scheduler reinitialized"
                            )
                        else:
                            self.logger.info(
                                f"Warmup complete: LR = {self.base_lr:.2e}, scheduler starting"
                            )
    ...
    def _generate_training_outputs(self):
        ...
        try:
            ...
            create_training_summary(
                self.config, self.training_history, self.config.output_dir
            )
            self.logger.info("Output: training summary generated")
            ...
            self.logger.info("Output: training metrics saved to root")
    ...
    def _setup_training_components(self):
        ...
        if self.config.pretrained_model_path and os.path.exists(
            self.config.pretrained_model_path
        ):
            try:
                self.logger.info(
                    f"Model: loading pretrained weights from {self.config.pretrained_model_path}"
                )
                ...
                self.logger.info("Model: pretrained weights loaded successfully")

                # Check if loaded model has NaN weights - if so, we can't use it
                has_nan = any(
                    torch.isnan(param).any() for param in self.model.parameters()
                )
                if has_nan:
                    self.logger.error(
                        "Model: pretrained weights contain NaN - cannot use for finetuning"
                    )
                    ...
                    if current_num_classes != target_num_classes:
                        self.logger.info(
                            f"Model: adapting from {current_num_classes} to {target_num_classes} classes"
                        )

                        # Replace the output layer to match the target number of classes
                        kernel_root = getattr(self.config, "kernel_root", 16)
                        self.model.out_layer = nn.Conv2d(
                            kernel_root, target_num_classes, 3, 1, 1
                        ).to(self.device)
                        ...
                        self.logger.info(
                            f"Model: successfully adapted to {target_num_classes} classes"
                        )
        ...
        if learning_rate is not None and learning_rate > 5e-4:
            self.logger.warning(
                f"Training: learning rate {learning_rate:.2e} reduced to 5e-4 to prevent NaN"
            )
            learning_rate = 5e-4
        ...
        if training_mode == "scratch":
            ...
            if not (
                self.config.pretrained_model_path
                and os.path.exists(self.config.pretrained_model_path)
                and pretrained_loaded
            ):
                raise ValueError("Continual learning requires a valid pretrained model")
            self.logger.info(
                "Training: continual learning mode (preserving optimizer state)"
            )
            ...
            raise ValueError(
                f"Invalid training_mode '{training_mode}'. Use: scratch, continual_learning, or fine_tuning"
            )
        ...
        if self.warmup_epochs > 0:
            ...
            self.logger.info(
                f"Training: warmup enabled for {self.warmup_epochs} epochs (LR: {self.warmup_start_lr:.2e} → {self.base_lr:.2e})"
            )
        ...
        (
            ...
            self.test_loader,
            test_files,
        ) = prepare_data_loaders(self.config, self.logger)
        if test_files:
    ...
    def _train_epoch(self) -> Tuple[float, float]:
        ...
        for batch in pbar:
            ...
            if isinstance(batch, dict) and "image" in batch and "label" in batch:
                ...
                batch_loss, _batch_dice, num_slices = 0.0, 0.0, 0
                batch_predictions = []
                batch_targets = []
                ...
                for b in range(images.shape[0]):
                    ...
                    loss, pred_logits, target = self._process_slice(
                        img_block, label_block, training=True
                    )
                    batch_loss += loss
                    num_slices += 1
            ...
            # Handle batch of BlockDataset objects (file-based mode)
                ...
                for block_dataset in batch:
                    ...
                    volume_loss, _volume_dice, num_slices = 0.0, 0.0, 0
                    ...
                    if num_slices > 0:
                        ...
                        if volume_predictions and volume_targets:
                            ...
                            stacked_predictions = torch.stack(
                                volume_predictions, dim=0
                            )  # [num_slices, 2, H, W]
                            stacked_targets = torch.stack(
                                volume_targets, dim=0
                            )  # [num_slices, H, W]
            ...
            for batch in pbar:
                ...
                if isinstance(batch, dict) and "image" in batch and "label" in batch:
                    ...
                    for b in range(images.shape[0]):
                        ...
                        loss, pred_logits, target = self._process_slice(
                            img_block, label_block, training=False
                        )
                        batch_loss += loss
                        num_slices += 1
                ...
                # Handle batch of BlockDataset objects (file-based mode)
                    ...
                    # Process each BlockDataset in the batch
                    for block_dataset in batch:
                        slice_data, slice_list, _ = block_dataset.get_one_directory(
                            axis=0
                        )
    ...
    def _setup_continual_learning(self):
        ...
        try:
            ...
            if "optimizer_state_dict" in checkpoint:
                ...
                self.logger.info(
                    "Training: loaded optimizer state (preserves momentum)"
                )
                ...
                if self.config.learning_rate is not None:
                    ...
                    for param_group in self.optimizer.param_groups:
                        ...
                        self.logger.info(
                            f"Training: LR overridden {old_lr:.2e} -> {param_group['lr']:.2e}"
                        )
                else:
                    # Preserve the learning rate from checkpoint
                    current_lr = self.optimizer.param_groups[0]["lr"]
                    self.logger.info(
                        f"Training: preserving checkpoint LR {current_lr:.2e}"
                    )
            else:
                self.logger.warning(
                    "Training: no optimizer state found - using fresh optimizer"
                )
            ...
            if (
                ...
                try:
                    ...
                    self.logger.warning(
                        f"Training: failed to load scheduler state - {e}"
                    )
    ...
    def _process_slice(
        self, img_block, label_block, training=True
        ...
        if target.max() >= num_classes:
            self.logger.error(
                f"Data: target contains invalid class labels (max={target.max()}, expected < {num_classes})"
            )
        ...
        if target.min() < 0:
            self.logger.error(
                f"Data: target contains negative class labels (min={target.min()})"
            )
            self.logger.error(f"Data: target unique values: {torch.unique(target)}")
            return 0.0, None, None
        ...
        if self.scaler is not None and training:
            with autocast(device_type="cuda"):
                ...
                if torch.isnan(output).any() or torch.isinf(output).any():
                    self.logger.error(
                        "Model: NaN/Inf detected in output - indicates model corruption"
                    )
                    if training:
            ...
            if (
                hasattr(self.config, "gradient_clip_norm")
