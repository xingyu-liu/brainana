...
def split_data(config, val_split=0.2, test_split=0.0, seed=42, min_train_subjects=5):
    """
    ...
    if total_subjects < min_required:
        print(
            f"Warning: Not enough subjects for splitting (need at least {min_required})"
        )
        ...
        if test_split > 0:
            ...
            if num_train < min_train_subjects:
                ...
                num_val = max(
                    1, int(remaining * (val_split / (val_split + test_split)))
                )
...
def main():
    ...
    val_split = (
        ...
        else getattr(config, "validation_split", 0.2)
    )
    test_split = (
        args.test_split
        if args.test_split is not None
        else getattr(config, "test_split", 0.0)
    )
