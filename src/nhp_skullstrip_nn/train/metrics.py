...
def prepare_tensors(
    ...
    with torch.no_grad():
        ...
        if target_classes.dim() == 3:  # (N, H, W) -> (N, C, H, W)
            target_onehot = (
                F.one_hot(target_classes, num_classes=num_classes)
                .permute(0, 3, 1, 2)
                .float()
            )
            ...
            raise ValueError(
                f"Expected 3D or 4D target tensor, got {target_classes.dim()}D"
            )
...
def compute_metrics_per_class(
    ...
    """
    with torch.no_grad():
        binary_pred, target_onehot = prepare_tensors(
            pred_logits, target_classes, num_classes, threshold
        )

        if compute_per_slice and binary_pred.dim() == 5:  # 3D volume case
            return _compute_metrics_per_slice(
                binary_pred, target_onehot, metric_type, num_classes
            )
        else:
            return _compute_metrics_volume(
                binary_pred, target_onehot, metric_type, num_classes
            )
...
def _compute_metrics_per_slice(
    ...
    for class_idx in range(num_classes):
        class_metrics = [
            slice_metrics[class_idx] for slice_metrics in all_slice_metrics
        ]
...
def compute_dice(
    ...
    return compute_metrics_per_class(
        pred_logits, target_classes, "dice", num_classes, threshold, compute_per_slice
    )
...
def compute_iou(
    ...
    return compute_metrics_per_class(
        pred_logits, target_classes, "iou", num_classes, threshold, compute_per_slice
    )
...
class DiceScore:
    ...
    This matches fastsurfer_nn's approach: accumulates statistics and computes dice
    ...
    def __init__(
        ...
        if device is not None:
            self.union = torch.zeros(
                num_classes, num_classes, device=device, dtype=torch.float32
            )
    ...
    def _convert_predictions(self, pred_logits: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_argmax:
            # Use argmax: matches fastsurfer_nn and standard inference
    ...
    def update(self, pred_logits: torch.Tensor, target_classes: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            pred_classes = self._convert_predictions(
                pred_logits
            )  # [N, H, W] or [N, H, W, D]
            ...
            union = gt_sums.unsqueeze(1) + pred_sums.unsqueeze(
                0
            )  # Broadcasting to [num_classes, num_classes]
...
class MetricsTracker:
    ...
    Now uses fastsurfer_nn's accumulated approach: accumulates intersection/union
    ...
    def __init__(
        ...
        # Initialize DiceScore for accumulated computation (fastsurfer_nn approach)
    ...
    def update(
        ...
        Note: For accumulated dice computation (fastsurfer_nn approach), pass pred_logits and target_classes.
        The dice will be computed at epoch end from accumulated statistics.
        """
        if loss is not None:
            self.losses.append(loss)

        # Update accumulated dice score (fastsurfer_nn approach)
        ...
        if pred_logits is not None and target_classes is not None and iou is None:
            iou = compute_foreground_iou(
                pred_logits, target_classes, compute_per_slice=self.compute_per_slice
            )
    ...
    def get_averages(self) -> Dict[str, float]:
        """Get average metrics."""
        # Compute dice from accumulated statistics (fastsurfer_nn approach)
        dice = self.get_dice()

        return {
            "loss": sum(self.losses) / len(self.losses) if self.losses else 0.0,
            "dice": dice,
            "iou": sum(self.iou_scores) / len(self.iou_scores)
            if self.iou_scores
            else 0.0,
        }
    ...
    def get_dice(self) -> float:
        """
        Get dice score from accumulated statistics (fastsurfer_nn approach).
    ...
    def get_dice_per_class(self) -> Tuple[List[float], torch.Tensor]:
        ...
        """
        dice_per_class, dice_matrix = self.dice_score.compute(
            per_class=True, exclude_background=False
        )
...
def create_metrics_tracker(
    compute_per_slice: bool = False, logger=None
) -> MetricsTracker:
