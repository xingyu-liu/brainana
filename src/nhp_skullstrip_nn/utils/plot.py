...
def plot_volume(
    volume_data, slice_idx=None, title="Volume", cmap="gray", figsize=(12, 8)
):
    """
...
def create_mri_image(
    ...
    if contour_data is not None:
        if isinstance(contour_data, (str, Path)):
            contour, contour_voxel_sizes, contour_orientation_code = _load_image(
                contour_data
            )
            # if 4d, get tmean image
            if contour.ndim == 4:
    ...
    if underlay_vmin is None or underlay_vmax is None:
        ...
        underlay_for_percentile = (
            underlay.astype(float) if underlay.dtype == bool else underlay
        )
        # Use percentile-based scaling to avoid outliers
        underlay_vmin = (
            np.percentile(underlay_for_percentile, 1)
            if underlay_vmin is None
            else underlay_vmin
        )
        underlay_vmax = (
            np.percentile(underlay_for_percentile, 99)
            if underlay_vmax is None
            else underlay_vmax
        )
        ...
        for col, slice_idx in enumerate(slice_indices):
            ...
            if overlay is not None:
                ...
                if np.any(overlay_slice != 0):
                    plotted_labels = _plot_labeled_data(
                        ax, overlay_slice, overlay_colors, "overlay", overlay_alpha
                    )
            ...
            if contour is not None:
                ...
                if np.any(contour_slice != 0):
                    plotted_labels = _plot_labeled_data(
                        ax, contour_slice, contour_colors, "contour", contour_alpha
                    )
                    # Collect labels for legend (only from first slice to avoid duplicates)
                    if row == 0 and col == 0:
