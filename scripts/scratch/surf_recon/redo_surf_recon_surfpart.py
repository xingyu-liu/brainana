...
for site_dir in site_list[::-1]:
    ...
    fastsurfer_dir = site_dir / "fastsurfer"
    if not fastsurfer_dir.exists():
        print("fastsurfer dir not found")
        continue
    ...
    for sub_dir in sub_list:
        ...
        if fs_sub_touch_dir.exists() and not overwrite_existing:
            print(
                f"  --> existing {site_name} / {sub} fastsurfer sub dir found, skipping"
            )
            continue
        ...
        try:
            qc_surf_recon_tissue_seg(
                ...
                output_path=sub_dir
                / "figures"
                / f'{str(anat_file.name).replace("_desc-preproc_T1w.nii.gz", "_desc-surfReconTissueSeg_T1w.png")}',
                modality="anat",
                config=config,
            )
        except Exception as e:
        ...
        try:
            qc_cortical_surf_and_measures(
                ...
                output_path=sub_dir
                / "figures"
                / f'{str(anat_file.name).replace("_desc-preproc_T1w.nii.gz", "_desc-corticalSurfAndMeasures_T1w.png")}',
                modality="anat",
                config=config,
            )
