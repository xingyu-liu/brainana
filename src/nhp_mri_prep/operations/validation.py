"""
Input validation and file system utilities for nhp_mri_prep.
...
def validate_input_file(imagef: Union[str, Path], logger: logging.Logger) -> Path:
    ...
    if not str(image_path).endswith(valid_extensions):
        raise ValueError(
            f"Input file must be a NIFTI file (.nii, .nii.gz) or transformation matrix (.mat): {image_path}"
        )
...
def validate_working_directory(
    working_dir: Union[str, Path], logger: logging.Logger
) -> Path:
    ...
    if work_dir.exists():
        ...
        try:
            ...
            raise PermissionError(
                f"Cannot write to existing working directory {work_dir}: {e}"
            )
    ...
    try:
        ...
        test_file.unlink()
        logger.debug(
            f"System: working directory path validated (will be created when needed) - {work_dir}"
        )
        return work_dir
    except Exception as e:
...
def ensure_working_directory(
    ...
    try:
        ...
        raise PermissionError(
            f"Cannot create or write to working directory {work_dir}: {e}"
        )
...
def validate_output_file(output_path: Union[str, Path], logger: logging.Logger) -> Path:
    ...
    if not str(output_file).endswith(valid_extensions):
        raise ValueError(
            f"Output file must be a NIFTI file (.nii, .nii.gz) or transformation matrix (.mat): {output_file}"
        )
