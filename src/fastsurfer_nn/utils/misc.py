...
import fastsurfer_nn.data_loader.loader
...
def plot_predictions(
    ...
    images_batch : torch.Tensor
        Batch of images.
    labels_batch : torch.Tensor
        Batch of labels (dense indices).
    batch_output : torch.Tensor
        Batch of output (dense indices).
    plt_title : str
        Plot title.
    file_save_name : str
...
def update_num_steps(
    dataloader: "fastsurfer_nn.data_loader.loader.DataLoader", cfg: yacs.config.CfgNode
) -> tuple[int, int]:
    """
    ...
    Parameters
    ----------
    dataloader : fastsurfer_nn.data_loader.loader.DataLoader
