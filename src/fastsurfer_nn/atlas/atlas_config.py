...
def create_freesurfer_config(plane: str = "coronal") -> AtlasConfig:
    ...
    return AtlasConfig(name="FreeSurfer", ctx_thresh=ctx_thresh, use_threshold=True)
    ...
    if cache_key not in _ATLAS_CONFIGS:
        if atlas_name.lower() == "arm3":
            if ARM3_CONFIG_PATH.exists():
                _ATLAS_CONFIGS[cache_key] = load_atlas_config_from_roiinfo(
                    ARM3_CONFIG_PATH
                )
            else:
...
def load_sagittal_hemisphere_mapping(
    roiinfo_path: Optional[Path] = None,
    ...
    sagittal_labels = sorted(
        [k for k in key_nohemi_set if k != 0], key=lambda x: (x < 0, abs(x))
    )
...
def create_sagittal_sparse_to_dense_mapping(
    roiinfo_path: Optional[Path] = None,
) -> Dict[int, int]:
    """
    ...
    """
    bilateral_to_merged, sagittal_labels, _ = load_sagittal_hemisphere_mapping(
        roiinfo_path
    )
...
def map_labels_to_sagittal_dense(
    label_array: np.ndarray, roiinfo_path: Optional[Path] = None
) -> np.ndarray:
    """
...
try:
    ...
    ARM3_SAGITTAL_REGION_NAMES = ["Background"] + [
        f"ARM3-{label}" for label in sagittal_labels
    ]
except Exception:
...
def map_labels_to_dense(
    label_array: np.ndarray, sparse_to_dense_map: Dict[int, int]
) -> np.ndarray:
    """
...
def map_labels_to_sparse(
    dense_array: np.ndarray, dense_to_sparse_map: np.ndarray
) -> np.ndarray:
    """
...
def generate_colorlut_from_roiinfo(roiinfo_path: Path, output_tsv_path: Path):
    ...
    bg_row = pd.DataFrame(
        [{"ID": 0, "LabelName": "Background", "R": 0, "G": 0, "B": 0, "A": 0}]
    )
    ...
    print("   Format: Extended (includes all roiinfo columns)")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--generate", action="store_true", help="Generate labels.json and ColorLUT.tsv"
    )
    parser.add_argument("--test", action="store_true", help="Run test suite")
    parser.add_argument(
        "--verify", type=str, help="Verify cortex detection on segmentation file"
    )
    ...
    if args.generate:
        ...
        generate_colorlut_from_roiinfo(roiinfo_path, current_dir / "ARM3_ColorLUT.tsv")
        print("\n✅ Done!")
...
def get_sagittal_to_bilateral_expansion(
    roiinfo_path: Optional[Path] = None,
) -> List[int]:
    """
    ...
    (
        bilateral_to_merged,
        sagittal_labels,
        num_sagittal_classes,
    ) = load_sagittal_hemisphere_mapping(roiinfo_path)
    ...
    bilateral_sparse_to_dense = create_sparse_to_dense_mapping(
        ARM3_LABELS
    )  # Use ARM3_LABELS explicitly
