...
def _compute_enorm_from_motion_params(
    ...
    if missing_cols:
        raise ValueError(f"Missing required motion columns for enorm calculation: {missing_cols}")
...
def apply_segmentation(
    ...
    logger.info(f"Workflow: starting segmentation using fastsurfer_nn")
    logger.info(f"Data: input image - {os.path.basename(image_path)}")
    ...
    try:
        ...
        if not fastsurfercnn_mask_path or not os.path.exists(fastsurfercnn_mask_path):
            raise FileNotFoundError(f"fastsurfer_nn did not generate brain mask at expected location: {fastsurfercnn_mask_path}")
...
def apply_skullstripping(
    ...
    try:
        ...
        if not nhp_mask_path or not os.path.exists(nhp_mask_path):
            raise FileNotFoundError(f"nhp_skullstrip_nn did not generate brain mask at expected location: {nhp_mask_path}")
