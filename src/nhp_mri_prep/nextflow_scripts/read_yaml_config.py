...
def main():
    if len(sys.argv) < 3:
        print(
            "Usage: read_yaml_config.py <config_file> <key_path> [default_value] [--type=TYPE]",
            file=sys.stderr,
        )
        print(
            "   or: read_yaml_config.py <config_file> <key1> <key2> ... [--defaults=val1,val2,...]",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    is_batch_mode = len(keys) > 1 or (
        len(sys.argv) > 3 and "--defaults=" in " ".join(sys.argv)
    )
    ...
    if is_batch_mode:
        ...
        for key, default in zip(keys, defaults):
            ...
            if isinstance(value, bool):
                ...
                if default.lower() in ("true", "false") and str_value.lower() in (
                    "true",
                    "false",
                    "1",
                    "0",
        ...
        if value_type == "bool":
            ...
            result = (
                ...
                else (str(default_value) if default_value is not None else "")
            )
        else:
            result = (
                str(value)
