...
def evaluate_test_set_from_training(
    ...
    if model_path is None:
        ...
        if model_path is None:
            raise FileNotFoundError(
                f"No model checkpoint found in {training_output_dir}"
            )
    ...
    for i, (image_path, label_path) in enumerate(zip(test_images, test_labels)):
        ...
        if logger:
            print(
                f"🔄 {i+1}/{total_files}: {os.path.basename(image_path)}, Dice: {result['dice_score']:.2f}, IoU: {result['iou_score']:.2f}"
            )
    ...
    if logger:
        ...
        logger.info(
            f"📈 Mean Dice: {summary_stats['mean_dice']:.2f} ± {summary_stats['std_dice']:.2f}"
        )
        logger.info(
            f"📈 Mean IoU: {summary_stats['mean_iou']:.2f} ± {summary_stats['std_iou']:.2f}"
        )

    return {"results": results, "summary": summary_stats, "eval_dir": eval_dir}


def evaluate_test_set_from_paths(
    ...
    for i, (image_path, label_path) in enumerate(zip(test_images, test_labels)):
        if logger:
            logger.info(
                f"🔄 Processing {i+1}/{total_files}: {os.path.basename(image_path)}"
            )
    ...
    return {"results": results, "summary": summary_stats, "eval_dir": output_dir}
...
def evaluate_single_volume(
    ...
    try:
        ...
        metrics_result = predict_volumes(
            ...
            ),  # Use num_classes from checkpoint config
        ...
        if metrics_result and base_name in metrics_result:
            ...
            dice_score = volume_metrics.get(
                "dice",
                volume_metrics.get(
                    "mean_dice", volume_metrics.get("brain_tissue_dice", 0.0)
                ),
            )
...
def generate_evaluation_summary(
    results: List[Dict], eval_dir: str, logger=None
) -> Dict:
    ...
    [r["file_name"] for r in successful_results]
...
def create_text_summary_report(results: List[Dict], stats: Dict, eval_dir: str):
...
def create_summary_plots(successful_results: List[Dict], stats: Dict, eval_dir: str):
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(eval_dir, "metrics_distribution.png"), dpi=300, bbox_inches="tight"
    )
    plt.close()
    ...
    # Dice scores
    ax1.bar(x_pos, dice_scores, alpha=0.7, color="skyblue")
    ...
    # IoU scores
    ax2.bar(x_pos, iou_scores, alpha=0.7, color="lightcoral")
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(eval_dir, "individual_performance.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
Examples:
  ...
  evaluate_test_set.py --test-images /path/to/test/images --test-labels /path/to/test/labels --model /path/to/model.pth --output /path/to/results
    ...
    parser.add_argument(
        "--model", help="Path to model checkpoint (auto-detected if not provided)"
    )
    parser.add_argument(
        "--output", help="Output directory (required with --test-images mode)"
    )
    parser.add_argument(
    ...
    try:
        if args.training_output:
            ...
            if len(test_images) != len(test_labels):
                print(
                    f"Warning: Mismatch in number of images ({len(test_images)}) and labels ({len(test_labels)})"
                )
