...
class StepOutput:
    ...
    def __post_init__(self):
        ...
        self.qc_files = [Path(f) if isinstance(f, str) else f for f in self.qc_files]
...
class AnatomicalState:
    ...
    def current(self) -> Path:
        ...
        return (
            self.registered
            or self.skullstripped
            or self.bias_corrected
            or self.conformed
            or self.reoriented
            or self.original
        )
...
class FunctionalState:
    ...
    def __post_init__(self):
        ...
        for attr in [
            ...
            "tmean_reoriented",
            "tmean_slice_timed",
            "tmean_motion_corrected",
            "tmean_despiked",
            "tmean_bias_corrected",
            "tmean_conformed",
            "tmean_skullstripped",
            "tmean_registered",
            "brain_mask",
    ...
    def current_tmean(self) -> Optional[Path]:
        ...
        return (
            self.tmean_registered
            or self.tmean_skullstripped
            or self.tmean_conformed
            or self.tmean_bias_corrected
            or self.tmean_despiked
            or self.tmean_motion_corrected
            or self.tmean_slice_timed
            or self.tmean_reoriented
        )
