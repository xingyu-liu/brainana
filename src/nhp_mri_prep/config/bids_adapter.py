...
def update_config_from_bids_metadata(
    ...
    if not _is_valid_slice_timing_data(slice_times, tr):
        ...
        logger.warning(
            f"Config: slice timing longer than repetition time ({np.max(slice_times)} > {tr}) - slice timing correction disabled"
        )
        return config
    ...
    if slice_direction:
        ...
        logger.info(
            "Config: slice timing correction updated from BIDS metadata and enabled"
        )
...
def _is_valid_slice_timing_data(slice_times: Any, tr: Any) -> bool:
    ...
    if (
        not slice_times
        or not isinstance(slice_times, (list, tuple))
        or len(slice_times) == 0
    ):
        return False
...
def _determine_slice_encoding_direction(metadata: Dict[str, Any]) -> Optional[str]:
    ...
    if orientation and len(orientation) >= 6:
        try:
            ...
            slice_direction_vector = np.cross(
