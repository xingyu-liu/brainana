...
class TrainingConfig:
    ...
    use_inst_norm: bool = (
        True  # Use instance normalization (disabled for medical imaging consistency)
    )
    ...
    # ============================================================================
    training_mode: str = (
        "fine_tuning"  # Training mode: 'scratch', 'fine_tuning', 'continual_learning'
    )
    ...
    enable_dynamic_batch_sizing: bool = (
        False  # Enable automatic batch size adjustment based on available memory
    )
    ...
    # Loss function
    loss_type: str = (
        "crossentropy"  # Loss function type: 'dice', 'bce', 'combined', 'crossentropy'
    )

    # Learning rate scheduling
    use_cosine_scheduler: bool = True  # Use cosine annealing scheduler
    scheduler_factor: float = (
        0.5  # ReduceLROnPlateau factor (only used when use_cosine_scheduler=False)
    )
    ...
    early_stopping_restore_best_weights: bool = (
        True  # Restore best weights on early stopping
    )
    ...
    batch_increase_factor: float = (
        1.2  # Factor to increase batch size when memory available
    )
    ...
    validation_metrics: Optional[
        Dict[str, bool]
    ] = None  # Validation metrics configuration
    ...
    log_level: str = (
        "DEBUG"  # Logging level, 'DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL'
    )
    ...
    augmentation_config: Optional[
        Dict[str, Any]
    ] = None  # Detailed augmentation settings
    ...
    track_training_metrics: bool = (
        False  # Track detailed training metrics (Dice, IoU, etc.)
    )
        ...
        if self.TRAINING_DATA_DIR:
            if not os.path.exists(self.TRAINING_DATA_DIR):
                raise FileNotFoundError(
                    f"Training data directory not found: {self.TRAINING_DATA_DIR}"
                )
    ...
    def get_data_paths(self) -> Tuple[List[str], List[str]]:
        ...
        if self.TRAINING_DATA_DIR.endswith(".json"):
            ...
            if "images" not in data or "labels" not in data:
                raise ValueError(
                    f"JSON file must contain 'images' and 'labels' keys: {self.TRAINING_DATA_DIR}"
                )
            ...
            if len(image_files) != len(label_files):
                raise ValueError(
                    f"Mismatch between number of images ({len(image_files)}) and labels ({len(label_files)}) in JSON file"
                )
            ...
            for img_file in image_files:
                ...
                label_basename = img_basename.replace(
                    ".nii.gz", f"_{self.label}.nii.gz"
                )
            ...
            if not valid_pairs:
                raise ValueError(
                    f"No valid image-label pairs found in {self.TRAINING_DATA_DIR}"
                )
