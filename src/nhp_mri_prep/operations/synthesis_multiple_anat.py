...
def synthesize_multiple_anatomical(
    ...
    logger.info(
        f"Reference image (run-{anat_files[0].run or '01'}): {anat_files[0].path}"
    )
    ...
    try:
        ...
        logger.info(
            f"Data: using reference {modality} - {os.path.basename(reference_path)}"
        )
        ...
        for i, anat_file in enumerate(anat_files[1:], 1):
            logger.info(
                f"Step: coregistering {modality} {i+1}/{len(anat_files)} - {os.path.basename(anat_file.path)}"
            )
            ...
            try:
                ...
                if not forward_transform:
                    raise RuntimeError(
                        "ANTs registration did not produce a forward transform"
                    )

                # Remove the output image, because we are going to recompute it
                registered_path = (
                    synthesis_work_dir / f"{output_prefix}_registered.nii.gz"
                )
                if registered_path.exists():
                    registered_path.unlink()

                # Apply transform (interpolation from config)
                interpolation = config.get("registration", {}).get(
                ...
                if not coregistered_path:
                    raise RuntimeError(
                        "ANTs apply transforms did not produce output image"
                    )

                logger.info(
                    f"Step: successfully coregistered - {os.path.basename(coregistered_path)}"
                )
                ...
                logger.error(
                    f"Step: coregistration failed for {os.path.basename(anat_file.path)} - {e}"
                )
                continue
        ...
        if len(coregistered_images) < len(anat_files):
            logger.warning(
                f"Data: only {len(coregistered_images)}/{len(anat_files)} images successfully coregistered"
            )

        if len(coregistered_images) > 1:
            logger.info(
                f"Step: averaging {len(coregistered_images)} coregistered images"
            )
            ...
            synthesized_img = nib.Nifti1Image(
                mean_data, affine=reference_img.affine, header=reference_img.header
            )
            ...
            logger.info(
                f"Output: synthesized {modality} saved - {os.path.basename(synthesized_path)}"
            )
            logger.info(
                f"Workflow: synthesis completed - {len(coregistered_images)}/{len(anat_files)} images successfully coregistered and averaged"
            )
            ...
            logger.info(
                f"Output: synthesis metadata saved - {os.path.basename(metadata_path)}"
            )
