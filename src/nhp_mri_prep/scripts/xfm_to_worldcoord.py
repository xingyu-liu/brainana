...
print(f"Original affine:\n{np.array2string(affine, precision=4, suppress_small=True)}")
...
target_voxel_size = np.round(np.min(voxel_sizes), 2)
print(f"Target isotropic voxel size: {target_voxel_size:.2f} mm")

# Create upright affine (identity rotation, isotropic scaling)
# Using minimum voxel size ensures we preserve the finest detail
# Isotropic scaling prevents stretching/distortion
upright_affine = np.eye(4)
upright_affine[:3, :3] = np.diag([target_voxel_size] * 3)
